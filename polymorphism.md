...
我们可以使用面向数据语言的伪代码从上面的代码中使用ECS 重新创建程序。
...
上面代码中有一个对象 `Ann`。该对象包含名字和姓氏（字符串），年龄（是数字）和布尔值（表明Ann不是成年人）。尽管对象中有不同的数据类型，但是对象对它们的处理方式没有差别。
...
我们在本文中简要地探讨了多态，本节将指出在处理 JavaScript 多态时要的一些注意事项。
