...
恰好此时调用栈中有函数需要执行，此时浏览器将会被阻塞，它不能够做其它任何事情。这意味着，浏览器会没有响应，不能够进行渲染和运行其它代码。这将会带来问题－你的程序界面将不再高效和令人愉悦。
...
什么是网页 API ？本质上，它们是你没有权限访问的线程，你只能够调用它们。它们是浏览器自带的，且可以在浏览器中进行并发操作。如果你是个 Node.js 开发者，这些是 C++ APIs。
...
需要注意的是 `setTimeout(…)` 并没有自动把你的回调添加到事件循环队列。它创建了一个定时器。当定时器过期，宿主环境会把回调函数添加至事件循环队列中，然后，在未来的某个 tick 取出并执行该事件。查看如下代码：
...
因此，以这样顺序执行的方式来表示你的异步代码看起来一气呵成，应该有这样的方法吧？
